...
def fetch_tx_reports_core_logic():
    ...
    try:
        ...
        if (
            res_tick.get("msg") == "success"
            and len(res_tick.get("data", [])) > 0
        ):

            df_tick = pd.DataFrame(res_tick["data"])

            tx_price = float(df_tick["price"].iloc[-1])

            open_p = float(df_tick["price"].iloc[0])

            high_p = float(df_tick["price"].max())

            low_p = float(df_tick["price"].min())
    ...
    try:
        ...
        if (
            res_daily.get("msg") == "success"
            and len(res_daily.get("data", [])) > 1
        ):
            ...
            y_high = float(yesterday["max"])
            y_low = float(yesterday["min"])
    ...
    try:
        ...
        param_oi = {
            "dataset": "TaiwanFuturesInstitutionalCommitment",
            "data_id": "TX",
            "start_date": start_str,
            "token": FINMIND_TOKEN
        }
        ...
        if (
            res_oi.get("msg") == "success"
            and len(res_oi.get("data", [])) > 0
        ):
            ...
            foreign_df = df_oi[
                df_oi["institutional_investors"]
                == "外資及陸資"
            ]

            if not foreign_df.empty:

                last_row = foreign_df.iloc[-1]

                foreign_net_oi = int(
                    last_row["open_interest_long"]
                    -
                    last_row["open_interest_short"]
                )
